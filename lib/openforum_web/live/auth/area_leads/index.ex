defmodule OpenforumWeb.Auth.AreaLeadLive.Index do
  ...
  def mount(_params, _session, socket) do
    socket =
      ...
      |> assign(:working_area, WorkingAreas.list_working_areas())
      |> assign(:area_lead, %AreaLead{})
  ...
  defp apply_action(socket, :edit, %{"id" => id}) do
    socket
    |> assign(:page_title, "Edit Area Lead")
    |> assign(:area_lead, AreaLeads.get_area_lead!(id))
  end

  defp apply_action(socket, :new, _params) do
    socket
    |> assign(:page_title, "New Area Lead")
    |> assign(:area_lead, %AreaLead{})
  end

  defp apply_action(socket, :index, _params) do
    socket
    |> assign(:page_title, "Area Lead")
    |> assign(:area_lead, nil)
    |> fetch_filtered_area_leads()
  end

  @impl true
  def handle_info(
        {OpenforumWeb.Auth.AreaLeads.FormComponent, {:saved, _area_lead}},
        socket
      ) do
    {:noreply, fetch_filtered_area_leads(socket)}
  end

  def handle_info(
        {OpenforumWeb.Auth.AreaLeads.FormComponent, {:cancelled, _}},
        socket
      ) do
  ...
  def update(%{area_lead: area_lead} = assigns, socket) do
    ...
    socket
    |> assign(assigns)
    |> assign_new(:area_leads, fn -> Openforum.Context.AreaLeads.list_area_leads() end)
    |> assign(:form, to_form(changeset))}
  end

  @impl true
  def handle_event("add_area_lead", _params, socket) do
    {:noreply,
     socket
     |> assign(:page_title, "New Area Leads")
     |> assign(:area_lead, %AreaLead{})
  ...
  def handle_event("edit", %{"id" => id}, socket) do
    {:noreply,
     socket
     |> assign(:page_title, "Edit area lead Area")
     |> assign(:area_lead, AreaLeads.get_area_lead!(id))
  ...
  defp fetch_filtered_area_leads(socket) do
    filters = %{
      ...
      <div class="flex flex-col gap-6 p-6">
        ...
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="text-[0.82rem] text-[#6B7280]">
              Manage Area Lead listings and their details.
            </p>
          </div>
          ...
          <div class="flex items-center gap-3">
            ...
            >
              <.icon name="hero-plus" class="size-4" /> New Area lead
            </button>
          </div>
        </div>
        ...
        <div class="overflow-hidden rounded-xl border border-[#E5E7EB] bg-white">
          <div class="border-b border-[#E5E7EB] px-5 py-4">
            <h2 class="text-[0.85rem] font-semibold text-[#0B2E4F]">All Area Leads</h2>
          </div>

          <Table.table id="tbl_area_leads" rows={@data}>
            <:col :let={area_lead} label="Name">
              <span class="font-medium text-[#252525]">{area_lead.name}</span>
            </:col>
            <:col :let={area_lead} label="Description">
              <span class="text-[#6B7280]">{area_lead.description}</span>
            </:col>

            <:action :let={area_lead}>
              <button
                phx-click="edit"
                phx-value-id={area_lead.id}
        ...
        <.modal
          ...
          id={"crud-area-lead-modal-#{(@area_lead && @area_lead.id) || :new}"}
          show
          on_cancel={JS.push("close")}
        >
          <.live_component
              module={OpenforumWeb.Auth.AreaLeads.FormComponent}
              id={@area_lead.id || :new}
              title={@page_title}
              action={@live_action}
              area_lead={@area_lead}
              area_leads={@area_leads}
              patch={~p"/admin/area_leads"}
            />
        </.modal>
      </div>
    </Layouts.admin_app>
