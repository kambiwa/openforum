defmodule OpenforumWeb.Auth.WorkingAreas.Index do
  ...
  def mount(_params, _session, socket) do
    socket =
      socket
      |> assign(:page_title, "Working Areas")
  ...
  defp apply_action(socket, :new, _params) do
    socket
    |> assign(:page_title, "New Working Area")
    |> assign(:working_area, %WorkingArea{})
  end
  ...
  defp apply_action(socket, :index, _params) do
    socket
    |> assign(:page_title, "Working Areas")
    |> assign(:working_area, nil)
    |> fetch_filtered_working_areas()
  end
  ...
  @impl true
  def handle_event("add_working_area", _params, socket) do
    {:noreply,
     socket
     |> assign(:page_title, "New Working Area")
     |> assign(:working_area, %WorkingArea{})
  ...
  defp fetch_filtered_working_areas(socket) do
    filters = %{
      ...
      <div class="flex flex-col gap-6 p-6">
        ...
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="text-[0.82rem] text-[#6B7280]">
              Manage working area listings and their details.
            </p>
          </div>
          ...
          <div class="flex items-center gap-3">
            ...
            >
              <.icon name="hero-plus" class="size-4" /> New Working Area
            </button>
          </div>
        </div>
        ...
        <div class="overflow-hidden rounded-xl border border-[#E5E7EB] bg-white">
          <div class="border-b border-[#E5E7EB] px-5 py-4">
            <h2 class="text-[0.85rem] font-semibold text-[#0B2E4F]">All Working Areas</h2>
          </div>

          <Table.table id="tbl_working_areas" rows={@data}>
            <:col :let={working_area} label="Name">
              <span class="font-medium text-[#252525]">{working_area.name}</span>
            </:col>
            <:col :let={working_area} label="Description">
              <span class="text-[#6B7280]">{working_area.description}</span>
            </:col>

            <:action :let={working_area}>
              <button
                phx-click="edit"
                phx-value-id={working_area.id}
        ...
        <.modal
          ...
          id={"crud-working-area-modal-#{(@working_area && @working_area.id) || :new}"}
          show
